...
{        
    ...
    public static void line(Graphics2D g, int x1, int y1, int x2, int y2) {   
        ...
        if ((x1 - x2) > 0) {line(g, x2, y2, x1, y1); return;}  
        ...
        if (Math.abs(y2 - y1) > Math.abs(x2 - x1)) {  
            ...
            steepline(g, y1, x1, y2, x2); return;   
        }  
    ...
    public static void splitOnBorder(Polygon polygon, List<Polygon> list, int level)
    {                
        boolean top = false;
        boolean right = false;
        ...
        for (int i=0; i<polygon.npoints; i++)
        {
            if (polygon.ypoints[i] < startY) { top = true; break; }
            if (polygon.xpoints[i] > endX) { right = true; break; }
            if (polygon.ypoints[i] > endY) { bottom = true; break; }        
            if (polygon.xpoints[i] < startX) { left = true; break; }
        }
